...
const AddUser = () => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch(
          `https://pjsofttech.in:15443/users/getAllUserByEmail?email=${getEmail()}`
        );
  ...
  const handleSubmit = async () => {
    ...
    if (userName.trim() === "") {
      ...
      try {
        const response = await fetch(
          `https://pjsofttech.in:15443/users/save?adminemail=${getEmail()}`,
          {
            method: "POST",
            headers: {
  ...
  const handleEditClickOpen = async (id) => {
    try {
      const response = await fetch(
        `https://pjsofttech.in:15443/users/getById/${id}`
      );
      const result = await response.json();
      setEditUser(result);
      setEditOpen(true);
  ...
  const handleUpdate = async () => {
    if (editUser.userName.trim() === "") {
      ...
      try {
        const response = await fetch(
          `https://pjsofttech.in:15443/users/update/${editUser.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `https://pjsofttech.in:15443/users/delete/${userIdToDelete}`,
        {
          method: "DELETE",
        }
      );
