    ...
    const fetchCategories = async () => {
      try {
        ...
        const response = await fetch(
          `https://pjsofttech.in:15443/categories/getAllCategoriesByEmail?email=${encodeURIComponent(
            email
          )}`
        );
  ...
  const handleSubmit = async () => {
    if (categoryName.trim() === "") {
      ...
      try {
        const email = getEmail();
        const response = await fetch(
          `https://pjsofttech.in:15443/categories/save?adminemail=${encodeURIComponent(
            email
          )}`,
          {
            method: "POST",
