    ...
    const fetchData = async () => {
        ...
        try {
            const [
                ...
                axios.get(`http://13.233.43.240:8085/getAdmissionsByTodayCount?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/getAdmissionsBy7daysCount?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/getAdmissionsBy30DaysCount?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/getAdmissionsBy365DaysCount?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/TotalAdmission?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/AdmissionInToDaysRevenue?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/AdmissionIn7DaysRevenue?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/AdmissionIn30DaysRevenue?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/AdmissionIn365DaysRevenue?institutecode=${institutecode()}`),
                axios.get(`http://13.233.43.240:8085/TotalAdmissionRevenue?institutecode=${institutecode()}`),
            ]);
...
`;
  return (
    <div maxWidth="xl" style={{ padding: '2px' }}>
      ...
      <Grid item xs={12} style={{ marginBottom: '16px', display: 'flex', justifyContent: 'center' }} mt={2}>
          <Paper style={{ padding: '15px', boxShadow: '0px 2px 5px rgba(0,0,0,0.2)', width: '100%' }}>
            <YearlyGraph />
          </Paper>
        </Grid>
