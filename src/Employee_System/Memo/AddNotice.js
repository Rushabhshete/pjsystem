...
export default function AddNotice() {
  ...
  const fetchNotices = async () => {
    try {
      const response = await axios.get(`https://pjsofttech.in:10443/notices/getallByInstitutecode?institutecode=${institutecode}`);
  ...
  const handleAddNotice = async () => {
    ...
    try {
      await axios.post(`https://pjsofttech.in:10443/notices/addnotice?institutecode=${institutecode}`, newNotice);
  ...
  const handleDeleteNotice = async () => {
    if (noticeToDelete) {
      try {
        await axios.delete(`https://pjsofttech.in:10443/notices/deleteNotice/${noticeToDelete}`);
  ...
  const handleUpdateNotice = async () => {
    ...
    try {
      await axios.put(`https://pjsofttech.in:10443/notices/updateNotice/${selectedNotice.nid}`, updatedNotice);
