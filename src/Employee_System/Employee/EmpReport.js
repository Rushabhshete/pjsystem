...
const EmpReport = () => {
  ...
  const fetchAllEmployees = useCallback(async () => {
    try {
      const response = await axios.get(`https://pjsofttech.in:10443/getNonDeleted?institutecode=${institutecode}`);
  ...
  const fetchFilteredUser = useCallback(async () => {
    try {
      let url = `https://pjsofttech.in:10443/getNonDeleted?institutecode=${institutecode}`;
      if (filter === "7") {
        url = `https://pjsofttech.in:10443/employees/last7days?institutecode=${institutecode}`;
      } else if (filter === "30") {
        url = `https://pjsofttech.in:10443/employees/lastMonth?institutecode=${institutecode}`;
      } else if (filter === "365") {
        url = `https://pjsofttech.in:10443/employees/lastYear?institutecode=${institutecode}`;
      }
  ...
  const fetchByDateRange = useCallback(async () => {
    if (startDate && endDate) {
      try {
        const response = await axios.get(`https://pjsofttech.in:10443/byDateRange?startDate=${startDate}&endDate=${endDate}&institutecode=${institutecode}`);
        if (response.status === 200 && response.data && Array.isArray(response.data)) {
          setUser(response.data);
        } else {
          console.error("Unexpected response format:", response);
