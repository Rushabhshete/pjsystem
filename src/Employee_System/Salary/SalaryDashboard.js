...
const SalaryDashBoard = () => {
  ...
  const [selectChartMonth, setSelectChartMonth] = useState(
    currentDate.getMonth() + 1
  );
  const [selectChartYear, setSelectChartYear] = useState(
    currentDate.getFullYear()
  );
  const [chart2Data, setChart2Data] = useState([]);
  const [selectedChart2Year, setSelectedChart2Year] = useState(
    currentDate.getFullYear()
  );
  ...
  const fetchEmployeesByMonthYear = async (month, year) => {
    try {
      const [
        ...
        paidEmployeesResponse,
        pendingEmployeesResponse,
        grossSalaryResponse,
        paidAmountResponse,
        pendingAmountResponse,
      ] = await Promise.all([
        axios.get("http://localhost:8082/salaries/salarycountbymonthyear", {
          params: { month, year, institutecode },
        }),
        axios.get("http://localhost:8082/salaries/Paidcountbyyearmonth", {
          params: { month, year, institutecode },
        }),
        axios.get("http://localhost:8082/salaries/Pendingcountbyyearmonth", {
          params: { month, year, institutecode },
        }),
        axios.get("http://localhost:8082/salaries/totalwithdrawalbymonthyear", {
          params: { month, year, institutecode },
        }),
        axios.get("http://localhost:8082/salaries/paidamountbymonthandyear", {
          params: { month, year, institutecode },
        }),
        axios.get(
          "http://localhost:8082/salaries/pendingamountbymonthandyear",
          { params: { month, year, institutecode } }
        ),
      ]);
  ...
  const fetchTotalEmployeesByYear = async (year) => {
    try {
      const response = await axios.get(
        "http://localhost:8082/salaries/salarycountbyyear",
        { params: { year, institutecode } }
      );
  ...
  const fetchChartData = async (year) => {
    try {
      const response = await axios.get(
        `http://localhost:8082/salaries/salariesbyYear?year=${year}&institutecode=${institutecode}`
      );
  ...
  useEffect(() => {
    const fetchData = async (month, year) => {
      try {
        const response = await axios.get(
          `http://localhost:8082/salaries/compareSalaryByMonth?month=${month}&year=${year}&institutecode=${institutecode}`
        );
  ...
  useEffect(() => {
    const fetchComparisonData = async (startYear, endYear) => {
      try {
        const response = await axios.get(
          `http://localhost:8082/salaries/yearlyFinalNetSalaryComparison`,
          {
            params: {
  ...
  const formattedComparisonData = comparisonData
    ? [
        {
          name: `Year ${selectedStartYear}`,
          value: comparisonData[selectedStartYear],
        },
        {
          name: `Year ${selectedEndYear}`,
          value: comparisonData[selectedEndYear],
        },
      ]
    : [];
        ...
        <Grid container spacing={2} mt={1}>
          ...
          <Grid item xs={12} md={4}>
            ...
            >
              ...
              <Typography variant="h5">
                {paidEmployees !== null ? paidEmployees : 0}
              </Typography>
            </Paper>
          </Grid>
          <Grid item xs={12} md={4}>
            ...
            >
              ...
              <Typography variant="h5">
                {pendingEmployees !== null ? pendingEmployees : 0}
              </Typography>
            </Paper>
          </Grid>
          <Grid item xs={12} md={4}>
          ...
          <Paper elevation={3} style={{ padding: "16px", height: "550px" }}>
            ...
            >
              ...
              <FormControl variant="outlined">
                <InputLabel id="select-chart-month-label">
                  Select Month
                </InputLabel>
                <Select
              ...
              <FormControl variant="outlined">
                <InputLabel id="select-chart-year-label">
                  Select Year
                </InputLabel>
                <Select
            ...
            >
              <div style={{ height: 400, width: "100%" }}>
                <ResponsiveBar
                  ...
                  colors={({ id, data }) =>
                    id === "currentMonthTotal" ? COLORS[0] : COLORS[1]
