...
//     setError(null);
//     try {
//       const response = await axios.get(`http://13.233.43.240:8082/salaries/byCategoryName/${categoryName}&institutecode=${institutecode}`);
...
//         </Grid>
//         <Typography variant="h6" align="right" padding={'5px'} border={'1px solid grey'}>
...
const EmpDetails = ({ empID }) => {
  ...
  const fetchUsers = async () => {
    ...
    try {
        const response = await axios.get(`http://13.233.43.240:8082/employees/status/${status}?institutecode=${institutecode}`);
        const formattedUsers = response.data.map(user => ({
            empID: user.empID,
            fullName: user.fullName,
            email: user.email,
