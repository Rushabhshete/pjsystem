...
export default function DashBoard() {
  ...
  const fetchEnquiriesByDateRange = async () => {
    if (startDate && endDate) {
      try {
        const response = await axios.get(`http://localhost:8086/get/getALLEnquiryByInstitutecode?institutecode=${institutecode}`);
        const data = response.data;
        const filteredEnquiries = data.filter(enquiry => {
          const enquiryDate = new Date(enquiry.enquiryDate).toISOString().split('T')[0];
  ...
  const [threeSixtyFiveDaysApplication, setThreeSixtyFiveDaysApplication] = useState(0);
  ...
  const periodData = () => {
    switch (selectedPeriod) {
      ...
      case 'Last 30 Days':
        return [
          { name: 'Total Enquiries', total: totalApplications, period: thirtyDaysApplication },
        ];
      case 'Last 365 Days':
        return [
          { name: 'Total Enquiries', total: totalApplications, period: threeSixtyFiveDaysApplication },
        ];
      default:
        return [];
    }
  ...
  useEffect(() => {
    const fetchEnquiries = async () => {
      try {
        ...
        const todaysResponse = await axios.get(`http://localhost:8086/getenquiryCount/today?institutecode=${institutecode}`);
  ...
  return (
    <div sx={{ padding: 2, width: "100%" }}>
      ...
      <Box mt={1} textAlign="center" sx={{ width: "100%" }}>
        ...
        <Grid container mt={1} spacing={3} justifyContent="center" className="textField-root">
