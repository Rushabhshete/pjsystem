...
export default function UpdateSource() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:8086/update/source/${id}`,
        conduct,
        {
          headers: {
      ...
      console.error("There was an error updating the source!", error);
    }
  };
    ...
    const loadUser = async () => {
      try {
        const result = await axios.get(
          `http://localhost:8086/get/source/${id}`,
          {
            headers: {
              "Content-Type": "application/json",
              "Access-Control-Allow-Origin": "*", // Add this header
        ...
        console.error("There was an error fetching the source!", error);
      }
    };
    loadUser();
  }, [id]);
